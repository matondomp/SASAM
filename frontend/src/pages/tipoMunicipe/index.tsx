...
export const TipoMunicipe: FC = () => {
  ...
  function initForm(item: any) {
        ...
        setId(typeMunicipe[item.key].id)

        forms.setFieldsValue({
          name:typeMunicipe[item.key].name,
          estado_id:typeMunicipe[item.key].estado_id,
        });
  ...
  const getTypeMunicipe = useCallback(async () => {
    ...
    const response = await api.post("/tipo-municipe/list", { filter: inputSearchValue })
    const { data } = response
  ...
  async function handleUpdate(data:any) {
    console.log(id)
    try {
      await api.put("/tipo-municipe/"+id, data)
  ...
  async function handleSubmit(data: any) {
    ...
    try {
      await api.post("/tipo-municipe", data)
