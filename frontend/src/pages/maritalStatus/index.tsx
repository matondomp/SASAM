...
export const MaritalStatus: FC = () => {
  ...
  function initForm(item: any) {
        ...
        setId(estadoCivil[item.key].id)

        forms.setFieldsValue({
          name:estadoCivil[item.key].name,
          estado_id:estadoCivil[item.key].estado_id,
        });
  ...
  const getEstadoCivil = useCallback(async () => {
    ...
    const response = await api.post("/estado-civil/list", { filter: inputSearchValue })
    const { data } = response
  ...
  async function handleUpdate(data:any) {
    console.log(id)
    try {
      await api.put("/estado-civil/"+id, data)
  ...
  async function handleSubmit(data: any) {
    ...
    try {
      await api.post("/estado-civil", data)
